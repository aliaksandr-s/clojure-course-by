...
(defn delete
  ...
  ([data & {:keys [where]}]
   (swap! data
          (fn [current-state] (remove where current-state)))))
...
(defn update
  ([data upd-map]
   (swap! data
          (fn [current-state] (map #(into % upd-map) current-state))))
  ([data upd-map & {:keys [where]}]
   (swap! data
          (fn [current-state] (map #(if (where %) (into % upd-map) %) current-state)))))
...
(defn insert [data new-entry]
  (swap! data 
         (fn [current-state] (conj current-state new-entry))))
